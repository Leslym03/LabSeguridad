...
def _repeated_seq_pos(text, seq_len):
    seq_pos = {}  # entradas de secuencia: [posiciones]
...
def _candidate_key_lengths(factor_lists, max_key_len):
    ...
    # excluir factores mayores que la longitud máxima de clave sospechada
...
def find_key_length(cyphertext, seq_len, max_key_len):
    # encontrar secuencias repetidas y sus posiciones
    rsp = _repeated_seq_pos(text=cyphertext, seq_len=seq_len)
    seq_spc = {}
    for seq, positions in rsp:
        seq_spc[seq] = _get_spacings(positions)
    # calcular los espacios entre las posiciones de cada repetido
     # secuencia y factorizar espaciamientos
    factor_lists = []
    for spacings in seq_spc.values():
        for space in spacings:
            factor_lists.append(_get_factors(number=space))
    # obtener factores comunes por frecuencia descendente,
     # que constituyen longitudes de clave candidatas
